...
export default function MessagesPage() {
  ...
  const [currentProfile, setCurrentProfile] = useState<{ full_name?: string } | null>(null);
...
const handleCallUser = async () => {
  ...
  const roomId = `conv-${selectedConversation.id}`;

  const supabase = createClient();

  // ✅ Use currentProfile here
  const callerName = currentProfile?.full_name || 'A user';

  // Send invitation
  const channel = supabase.channel(`user:${otherUserId}`);
  await channel.send({
    type: 'broadcast',
    event: 'call_invitation',
    payload: {
      caller_id: currentUserId,
      caller_name: callerName, // ✅ now defined
      room_id: roomId,
    },
  });
  ...
  setCallTarget({
    id: otherUserId,
    name: selectedConversation.other_user_full_name,
  });
};
...
>
  📞
     ...
     {callTarget && selectedConversation && currentUserId && (
  <HandleCall
    isOpen={true}
    onClose={() => setCallTarget(null)}
    roomId={`conv-${selectedConversation.id}`}
    participantName={callTarget.name}
